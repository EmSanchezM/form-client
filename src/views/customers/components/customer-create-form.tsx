...
import { useCreateForm } from '../hooks/use-create-customer';
...
const CustomerCreateForm: FC<CustomerCreateFormProps> = ({ closeModal }) => {
  const context = useContext(CustomerFormsContext);

  if (!context) {
    throw new Error('CustomerCreateForm debe ser usado dentro de un CustomerFormsContext.Provider');
  }
  ...
  const { isFormInvalid, firstName, lastName, birthday, gender, cellphone, homephone, profession, addressHome, incomes, dispatchFormValues } = useCreateForm();

  return (
    <form className={style.form} onSubmit={evt => handleSubmit(evt, firstName, lastName, birthday, gender, cellphone, homephone, profession, addressHome, incomes, handleIsSubmitting, onSuccess, closeModal)}>
      <InputText
        label='Nombre'
        placeholder='John'
      ...
      <div className={style.row}>
        ...
        <Button type='submit' kind='primary' disabled={isSubmitting || isFormInvalid === true}>
          {isSubmitting ? 'Cargando...' : 'Crear cliente'}
        </Button>
      </div>
...
const handleSubmit = async (
  ...
  const success = await createCustomer(customer);

  if (success) {
    onSuccess()
    toastAlert.success('Cliente creado con éxito');
  } else {
    toastAlert.error('Error al crear cliente');
  }
  closeModal();
};
